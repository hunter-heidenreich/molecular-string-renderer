...
class TestValidateMolecularString:
    ...
    def test_empty_molecular_strings(self):
        ...
        for molecular_string, format_type in empty_cases:
            with pytest.raises(ValidationError, match=f"Empty {format_type.upper()} string"):
                validate_molecular_string(molecular_string, format_type)
    ...
    def test_excessively_long_strings(self):
        ...
        with pytest.raises(ValidationError, match="too long.*Maximum supported length is 10,000"):
...
class TestValidateFormatType:
    ...
    def test_invalid_formats(self):
        ...
        for invalid_format in invalid_formats:
            with pytest.raises(ValidationError, match=f"Unsupported format type: '{invalid_format}'"):
    ...
    def test_error_message_includes_valid_formats(self):
        ...
        with pytest.raises(ValidationError, match="Supported formats: \\['inchi', 'selfies', 'smiles'\\]"):
...
class TestValidateGridParameters:
    ...
    def test_too_many_molecules(self):
        ...
        with pytest.raises(ValidationError, match="Too many molecules.*Maximum supported is 100"):
    ...
    def test_invalid_mol_size_structure(self):
        ...
        for invalid_size in invalid_sizes:
            with pytest.raises(ValidationError, match="must be a tuple of \\(width, height\\)"):
...
class TestValidateOutputPath:
    ...
    def test_directory_creation_os_error(self, mock_mkdir):
        ...
        with pytest.raises(ValidationError, match="Cannot create output directory.*Disk full"):
...
class TestValidateConfigurationCompatibility:
    ...
    def test_dimensions_too_small(self):
        ...
        with pytest.raises(ConfigurationError, match="Render dimensions too small.*Minimum is 50x50"):
            validate_configuration_compatibility(render_config, parser_config, output_config)
    ...
    def test_dimensions_too_large(self):
        ...
        with pytest.raises(ConfigurationError, match="Render dimensions too large.*Maximum is 5000x5000"):
            validate_configuration_compatibility(render_config, parser_config, output_config)
    ...
    def test_jpeg_transparency_conflict(self):
        ...
        with pytest.raises(ConfigurationError, match="JPEG format does not support transparent backgrounds"):
            validate_configuration_compatibility(render_config, parser_config, output_config)
    ...
    def test_quality_validation_for_jpeg_webp(self):
        ...
        for fmt in ["jpeg", "webp"]:
            ...
            with pytest.raises(ConfigurationError, match="Quality.*invalid.*Must be 1-100"):
                validate_configuration_compatibility(render_config, parser_config, output_config)
    ...
    def test_svg_line_width_validation(self):
        ...
        output_config = OutputConfig(format="svg", svg_line_width_mult=0.5)  # Valid initially
        output_config.svg_line_width_mult = 0  # Manually set invalid value
        with pytest.raises(ConfigurationError, match="SVG line width multiplier must be positive"):
            validate_configuration_compatibility(render_config, parser_config, output_config)
        
        # Test negative line width
        output_config = OutputConfig(format="svg", svg_line_width_mult=1.0)  # Valid initially
    ...
    def test_hydrogen_coordination_info(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert "render_config.show_hydrogen=True requires parser_config.show_hydrogen=True" in caplog.text
