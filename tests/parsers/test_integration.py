...
class TestSubModulePerformance:
    ...
    def test_parser_creation_speed(self):
        ...
        assert creation_time < 1.0, f"Parser creation took too long: {creation_time:.3f} seconds"
...
class TestSubModuleStructure:
    ...
    def test_module_has_correct_exports(self):
        ...
        if hasattr(parsers_module, '__all__'):
            for export in expected_exports:
                assert export in parsers_module.__all__, f"Export {export} not in __all__"
    
    def test_individual_modules_importable(self):
        """Test that individual sub-modules can be imported directly."""
        # These should all work without errors
        from molecular_string_renderer.parsers.base import MolecularParser as BaseMolecularParser
        from molecular_string_renderer.parsers.smiles import SMILESParser as BaseSMILESParser
        from molecular_string_renderer.parsers.factory import get_parser as base_get_parser
...
class TestDocumentationAndMetadata:
    ...
    def test_parser_classes_have_docstrings(self):
        ...
        for parser_class in parser_classes:
            assert parser_class.__doc__ is not None, f"{parser_class.__name__} missing docstring"
            assert len(parser_class.__doc__.strip()) > 0, f"{parser_class.__name__} has empty docstring"
