...
class TestGetParser:
    ...
    def test_case_insensitive_format_type(self):
        ...
        for format_type, expected_class in test_cases:
            ...
            assert isinstance(parser, expected_class), f"Failed for format: {format_type}"
    ...
    def test_whitespace_handling(self):
        ...
        for format_type in test_cases:
            parser = get_parser(format_type)
            assert isinstance(parser, SMILESParser), f"Failed for format: '{format_type}'"
...
class TestGetParserWithConfig:
    ...
    def test_config_passed_to_all_parser_types(self):
        ...
        for format_type in parser_formats:
            ...
            assert parser.config is custom_config, f"Config not passed for {format_type}"
            assert parser.config.sanitize is False, f"Config values not preserved for {format_type}"
...
class TestParserMapping:
    ...
    def test_all_aliases_mapped(self):
        ...
        for format_type, expected_class in expected_mappings.items():
            parser = get_parser(format_type)
            assert isinstance(parser, expected_class), f"Mapping failed for {format_type}"
