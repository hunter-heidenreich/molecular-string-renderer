...
class TestSMILESExceptionHandling:
    ...
    def test_rdkit_exception_reraising(self):
        ...
        with patch(
            "molecular_string_renderer.parsers.smiles.Chem.MolFromSmiles"
    ...
    def test_generic_exception_wrapping(self):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Failed to parse SMILES 'CCO': Some internal RDKit error",
            ):
                parser.parse("CCO")
...
class TestSMILESEdgeCases:
    ...
    def test_parse_isotope_molecules(self):
        ...
        deuterium_atoms = [
            atom for atom in atoms if atom.GetSymbol() == "H" and atom.GetIsotope() == 2
        ]
